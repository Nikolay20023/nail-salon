...
import { api, Service, Master, Booking, BookingCreate } from '../utils/api'
...
function UpdateBooking() {
  ...
  const loadBooking = async (bookingId: number) => {
    try {
      ...
      setSelectedMasterId(data.master_id || '')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!id || !selectedServiceId || !selectedMasterId || !selectedDate || !selectedTime) {
    ...
    try {
      const bookingUpdate: BookingCreate = {
        user_id: user.id,
      ...
      await api.updateBooking(Number(id), bookingUpdate)
      showAlert('Запись успешно обновлена!')
      navigate('/my-booking')
    } catch (err) {
      const message = err instanceof Error ? err.message : 'Ошибка при обновлении записи'
      setError(message)
      showAlert(message)
    } finally {
      setSaving(false)
  ...
  return (
    <div className="container">
      <h1 className="page-title">Изменить запись</h1>
